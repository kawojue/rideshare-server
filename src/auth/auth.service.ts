...
export class AuthService {
  ...
  async verifyGoogleSignin(
    req: Request,
    { idToken, ...device }: GoogleSigninDTO,
  ) {
    ...
    try {
      const tk = await this.client.verifyIdToken({
        ...
        audience:
          os === 'android'
            ? config.google.clientId.android
            : os === 'ios'
              ? config.google.clientId.ios
              : '',
      });
      ...
      const user = await this.prisma.user.findFirst({
        where: {
          OR: [{ email: payload.email }, { providerId: payload.sub }],
        },
        select: {
          id: true,
          role: true,
      ...
      const mobileDevice = await this.mobileDevice(
        req,
        user.id,
        device.deviceId,
  ...
  async verifySignin(
    req: Request,
    { otp, identifier, ...device }: VerifySigninDTO,
  ) {
    const isPhone = !isEmail(identifier);
    ...
    if (isPhone) {
      const { countryCode, regionCode, significant } =
        Utils.normalizePhoneNumber(identifier);
  ...
  async biometricSignin({ access_token: accessToken }: BiometricLoginDTO) {
    ...
    Utils.sanitizeData<typeof user>(user, [
      'providerId',
      'lastUsedBiometricAt',
    ]);
  ...
  async emergencyContact(
    { sub }: JwtDecoded,
    { fullname, phone, address, email }: EmergencyContactDTO,
  ) {
    if (!phone && !email) {
