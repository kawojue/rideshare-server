...
export class DriverService {
  ...
  private async verifyVehicleOwner(
    plateNumber: string,
    firstname: string,
    lastname: string,
  ) {
  ...
  async idVerification(
    ...
    if (idType === 'NIN') {
      ...
      data = await this.qoreid.votersCard(
        { idNumber },
        {
          lastname: verification.driver.lastname,
          firstname: verification.driver.firstname,
      ...
      data = await this.qoreid.passport(
        { idNumber },
        {
          lastname: verification.driver.lastname,
          firstname: verification.driver.firstname,
  ...
  async driverLicenseVerification(
    ...
    const data = await this.qoreid.driversLicense(
  ...
  async addVehicle(
    ...
    const { verified, metadata } = await this.verifyVehicleOwner(
      plateNumber,
    ...
    return await this.prisma.vehicle.create({
      data: {
        ...
        agreementDocument:
          isNotOwnerdata?.agreement as unknown as Prisma.JsonValue,
        amenity: {
          create: {
            wifi,
            music,
