...
import { FilterBy, SignupPromoDTO, FetchPromosDTO } from './dto/promo.dto';
import {
  Injectable,
  NotFoundException,
  ConflictException,
...
export class ModminService {
  ...
  async inviteNewModmin({
    ...
    if (isExists) {
      throw new ConflictException(
        `${Utils.titleText(isExists.role)} already exists`,
      );
    }
  ...
  async toggleAccountSuspension(accountId: string, { sub }: JwtDecoded) {
    ...
    const acc = await (
      this.prisma[
        `${account.role === 'ADMIN' || account.role === 'MODERATOR' ? 'modmin' : 'user'}`
      ] as any
    ).update({
      where: { id: accountId },
  ...
  async verifyProofOfAddress(
    ...
    this.event.emit(
      'notification.email',
      new CreateEmailNotificationEvent({
        template: data.addressVerified
          ? 'ProofOfAddressApproved'
          : 'ProofOfAddressDisapproved',
  ...
  async createPromo(
    { sub }: JwtDecoded,
    { code, title, expiry, reward, max = 50 }: SignupPromoDTO,
  ) {
