...
export class WalletService {
  ...
  async bankAccountVerification({
    ...
    const { data } = await this.paystack.resolveAccount(
      account_number,
      bank_code,
    );
    return data;
  ...
  async fetchBankByBankCode(bankCode: string) {
    ...
    if (!bank) {
      throw new NotFoundException(
        'No supported Bank Name is associated with this bank code.',
      );
    }
  ...
  async requestWithdrawal(
    ...
    const { data: destination } = await this.paystack.resolveAccount(
      account_number,
      bank_code,
    );
    ...
    if (!eligible) {
      throw new BadRequestException(
        "You're not yet eligible to request for a withdrawal",
      );
    }
  ...
  async fundWallet({ sub }: JwtDecoded, { reference }: FundWalletDTO) {
    let userMutex =
      (await this.store.get<Mutex>(`fund-wallet_${sub}`)) ?? new Mutex();
